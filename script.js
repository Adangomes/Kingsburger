...
async function processarResumoGeo() {
    ...
    try {
        ...
        if (data.features && data.features.length > 0) {
            ...
            taxaEntregaCalculada = parseFloat((TAXA_BASE + (dist * VALOR_POR_KM)).toFixed(2));
            mostrarResumoFinal();
        } else {
...
function enviarWhatsApp() {
    ...
    if(btnWhats) { btnWhats.innerText = "PROCESSANDO..."; btnWhats.disabled = true; }
    ...
    salvarPedidoFirebase({
        nome, rua, num, bairro, pag,
        subtotal: carrinho.reduce((acc, i) => acc + i.price, 0),
        totalGeral: carrinho.reduce((acc, i) => acc + i.price, 0) + taxaEntregaCalculada - descontoAplicado
